...
work both as SFC and VNF orchestrator and it allows to save definitions of both
of them as their update and deletion. We developed \harbor{} in a manner that it
...
\egresses{}. This component is a prototype of a fully featured orchestrator
...
operation. \harbor{} is able to deploy for a definition previous given and to
delete a running chain but munitor and scale features are really limited.
Actually, \harbor{} implementation allows only to check if a chain is deployed
or not as their components.

\harbor{} is implemented in Java using Spark to expose a REST API with with is
possible to handle SFCs. 
